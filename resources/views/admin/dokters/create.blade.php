...
<form method="POST" action="{{ route('admin.dokters.store') }}">
    @csrf
    ...
    <select name="poli_id" required>
        ...
        @foreach($polis as $p)
            <option value="{{ $p->id }}" {{ old('poli_id') == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
        @endforeach
    </select>

    <label>Hari Jadwal</label>
    <input type="text" name="schedule_day" placeholder="Senin / Selasa" value="{{ old('schedule_day') }}" required>
